/**
 * POST /api/auto-generate-receiver
 *
 * Receives AI-generated posts from GitHub Actions and writes them to the database.
 * Security: CRON_SECRET header validation + rate limiting + content validation.
 */
...
const MAX_POSTS_PER_HOUR = 40; // allows bulk 36-post historical fill
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    for (const post of posts) {
      ...
      // insert post - published=true so it shows on blog.html immediately
      try {
    ...
    console.error('[auto-generate-receiver] error:', e);
    return res.status(500).json({
      error: 'Internal server error',
      detail: e.message || String(e),
    });
  } finally {
    // do not end pool here — Vercel reuses the process across requests
  }
};
