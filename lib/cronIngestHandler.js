...
const TOPIC_STOP_WORDS = new Set([
  'about', 'after', 'amid', 'among', 'announced', 'around', 'baker', 'bakers',
  'baking', 'bakery', 'bread', 'cake', 'cakes', 'chef', 'chefs', 'cookie',
  'cookies', 'dessert', 'desserts', 'expected', 'global', 'guide', 'industry',
  'international', 'major', 'market', 'markets', 'march', 'month', 'news',
  'pastry', 'pastries', 'projected', 'recipe', 'recipes', 'scheduled', 'set',
  'shows', 'study', 'through', 'trend', 'trends', 'update', 'world', 'worldwide',
  'year', 'years'
]);
...
function topicTokens(text) {
  return String(text || '')
    ...
    .split(/\s+/)
    .map(t => t.replace(/^-+|-+$/g, ''))
    .filter(t => t.length >= 4 && !/^\d{4}$/.test(t) && !TOPIC_STOP_WORDS.has(t));
}
...
async function findSimilarPost({ title, excerpt, type }) {
  ...
  const result = await sql`
    ...
    LIMIT ${DEDUPE_RECENT_LIMIT}
  `;
  for (const row of result.rows || []) {
    const oldText = `${row.title || ''} ${row.excerpt || ''}`;
    if (topicSimilarity(text, oldText) >= 0.75) return row;
  }
  return null;
}
...
module.exports = async function cronIngestHandler(req, res) {
  ...
  try {
    ...
    for (const post of items) {
      ...
      const similar = await findSimilarPost({ title, excerpt, type });
      if (similar) {
        results.skipped++;
        results.errors.push({
          title,
          reasons: [`similar topic to post ${similar.id}: ${similar.title}`],
        });
        continue;
      }
